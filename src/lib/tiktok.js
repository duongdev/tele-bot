...
const downloadMedia = async (item) => {
  ...
  return `${folder}${item.id}.mp4`;
};

// url contains the url, watermark is a bool that tells us what link to use
const getVideo = async (url, watermark) => {
  const idVideo = await getIdVideo(url);
  const API_URL = `https://api22-normal-c-alisg.tiktokv.com/aweme/v1/feed/?aweme_id=${idVideo}&iid=7318518857994389254&device_id=7318517321748022792&channel=googleplay&app_name=musical_ly&version_code=300904&device_platform=android&device_type=ASUS_Z01QD&version=9`;
  ...
  if (!!res.aweme_list[0].image_post_info) {
    ...
    if (watermark) {
      if (
        ...
        video.download_addr.url_list.length > 0
      ) {
    ...
    if (urlMedia === null) {
      if (
        ...
        video.play_addr.url_list.length > 0
      ) {
...
async function downloadFromUrl(url) {
  const listVideo = [];
  const videoUrl = await getRedirectUrl(url);
  listVideo.push(videoUrl);
  console.log(chalk.green(`[!] Found ${listVideo.length} video(s)`));
  const retryUrls = getRetryUrls();
  listVideo.push(...retryUrls);
  let videoPath = "";
  for (const videoUrl of listVideo) {
    console.log(chalk.green(`[*] Downloading video from: ${videoUrl}`));
    const data = await getVideo(videoUrl, false);
    // check if video was deleted => data empty
    if (data == null) {
      continue; // Skip if the video couldn't be retrieved
    }
    ...
    await downloadMedia(data)
      .then((filePath) => {
        console.log(chalk.green(`[+] Downloaded successfully: ${data.id}.mp4`));
        videoPath = `downloads/${data.id}.mp4`;
        removeFromRetry(videoUrl); // Remove from retry.txt if successful
      })
      .catch((err) => {
        console.log(chalk.red("[X] Error: " + err));
...
async function downloadTikTok(url) {
  ...
  let retries = 10;
  while (!fs.existsSync(filePath) && retries > 0) {
    await new Promise((resolve) => setTimeout(resolve, 1000));
    retries--;
  }
